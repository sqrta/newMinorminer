...
int main(int argc, char *argv[]) {
   ...
   auto end = clockt::now();
    double duration = static_cast<double>(duration_cast<microseconds>(end - start).count());
    printf("%d iterations use %fs\n", max_it,duration/1e6);
    printf("minCount: %d\n", minCount);
    file.open("thread"+threadID, std::ios_base::app);
    file << dataID << " goodit: " << good_it << " time: " << duration/1e6 << "s for " << max_it << " iterations, avg: " << avg <<" thres: " << thres << " vars: " << triangle.num_nodes() << " machineNodes: "<< square.num_nodes() <<endl;
    return 0;
}
