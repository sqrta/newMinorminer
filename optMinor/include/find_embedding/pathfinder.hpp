...
class pathfinder_base : public pathfinder_public_interface {
  ...
  protected:
    ...
    int pushdown_overfill_pass(embedding_t &emb) {
        ...
        bool improved = false;
        auto order = ep.var_order();

        // vector<int> order={17, 18, 21, 22, 25, 26, 29, 30, 33, 34, 37, 38, 41, 42, 45, 46, 49, 50, 53, 54, 57, 58, 61, 62, 65, 66, 69, 70, 73, 74, 77, 78, 81, 82, 85, 86, 89, 90, 93, 94, 97, 98, 101, 102, 105, 106, 109, 110, 113, 114, 117, 118, 121, 122, 125, 126, 129, 130, 133, 134, 137, 138, 141, 142, 145, 146, 149, 150, 153, 154, 157, 158, 161, 162, 165, 166, 169, 170, 173, 174, 177, 178, 181, 182, 185, 186, 189, 190, 193, 194, 197, 198, 201, 202, 16, 19, 20, 23, 24, 27, 28, 31, 32, 35, 36, 39, 40, 43, 44, 47, 48, 51, 52, 55, 56, 59, 60, 63, 64, 67, 68, 71, 72, 75, 76, 79, 80, 83, 84, 87, 88, 91, 92, 95, 96, 99, 100, 103, 104, 107, 108, 111, 112, 115, 116, 119, 120, 123, 124, 127, 128, 131, 132, 135, 136, 139, 140, 143, 144, 147, 148, 151, 152, 155, 156, 159, 160, 163, 164, 167, 168, 171, 172, 175, 176, 179, 180, 183, 184, 187, 188, 191, 192, 195, 196, 199, 200, 203, 13, 14, 205, 206, 12, 15, 204, 207, 9, 10, 209, 210, 8, 11, 208, 211, 5, 6, 213, 214, 4, 7, 212, 215, 1, 2, 217, 218, 0, 3, 216, 219}; //data13
        ...
        for (auto &u : order) {
            if (pushback < num_vars) {
                ...
                emb.tear_out(u);
                if (!find_chain(emb, u, 0)) {
                    printf("%d fail cause pushdown fail\n", u);
                    return -1;
                }
            }
            improved |= check_improvement(emb);
  ...
  protected:
    ...
    void prepare_small_root_distances(const embedding_t &emb, const int u) {
        ep.prepare_distances(total_distance, u, max_distance);
        compute_qubit_weights(emb);
        ...
        int degree = ep.var_neighbors(u).size();
        for (auto &v : ep.var_neighbors(u)) {
            
            if (!emb.chainsize(v)) continue;
            int r = (int)(degree / 2);
            r = std::min(std::max(r, 5), 6);
            neighbors_embedded++;
            ep.prepare_visited(visited_list[v], u, v);
            compute_distances_from_chain(emb, v, visited_list[v], 4);
            accumulate_distance(emb, v, visited_list[v]);
        }

        if (!neighbors_embedded) {
            for (int q = num_qubits; q--;)
                if (emb.weight(q) >= ep.weight_bound) {
                    total_distance[q] = max_distance;
                } else {
                    distance_t d = max(qubit_weight[q], total_distance[q]);
                    total_distance[q] = d;
                }
        }
    }
    void compute_distances_from_chain(const embedding_t &emb, const int &v, vector<int> &visited, int disThres) {
        distance_queue pq(num_qubits);
        auto &parent = parents[v];
        auto &permutation = qubit_permutations[v];
        auto &distance = distances[v];
            ...
            for (auto &p : ep.qubit_neighbors(z.node)) {
                if (!visited[p]) {
                    ...
                    if (emb.weight(p) >= ep.weight_bound) {
                        ...
                        parent[p] = z.node;
                        pq.emplace(p, permutation[p], z.dist + qubit_weight[p], z.count+1);
                    }
                }
            }
        }
  ...
  public:
    ...
    virtual int heuristicEmbedding() override {
        ...
        ep.reset_mood();
        vector<pair<int, int>> degreePair;
        for (size_t i = 0; i < num_vars; i++)
        {
            degreePair.push_back(pair(i, ep.var_neighbors(i).size()));
        }
        auto degreeCompare = [](const pair<int, int> a, const pair<int, int> b) -> bool{
            return a.second > b.second;
        };
        std::stable_sort(degreePair.begin(), degreePair.end(), degreeCompare);
        for (auto i : degreePair)
        {
            degreeOrder.push_back(i.first);
            // printf("%d, ", i.first);
        }
        ...
        // {
        //     printf("i: %ld, degree: %ld\n", i, ep.var_neighbors(i).size());
        // }
        // return 1;
        ...
        for (int trial_patience = params.tries; trial_patience-- && (!ep.embedded);) {
            ...
            for (int round_patience = params.inner_rounds;
                ...
                // printf("r: %d, embed: %d, pushdown: %d, (%d more before giving up on this trial)\n", r, ep.embedded, pushback, min(improvement_patience, round_patience) - 1);
            }
...
class pathfinder_serial : public pathfinder_base<embedding_problem_t> {
  ...
  public:
    ...
    virtual void prepare_root_distances(const embedding_t &emb, const int u) override {
        ...
        int degree = super::ep.var_neighbors(u).size();
        for (auto &v : super::ep.var_neighbors(u)) {
            ...
            super::compute_distances_from_chain(emb, v, super::visited_list[v], super::thres[u]+r);
